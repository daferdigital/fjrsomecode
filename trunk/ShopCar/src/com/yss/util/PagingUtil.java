...
public final class PagingUtil {
	...
	public int getPageNumberRequested(){
		...
		if(request.getParameter(AppConstant.PAGING_PAGE_NUMBER) != null){
			try {
				pageNumber = Integer.parseInt(request.getParameter(AppConstant.PAGING_PAGE_NUMBER));
			} catch (Exception e) {
	...
	public String injectPageNumberAsHidden(){
		...
		final String hiddenField = "<input type=\"hidden\" name=\""
				+ AppConstant.PAGING_PAGE_NUMBER + "\" value=\""
				+ pageNumber + "\"/>";
	...
	public String getTRFooterPaging(){
		final SiteBean siteBean = (SiteBean) request.getAttribute(AppConstant.ATT_BEAN_SITE);
		...
		if(pageNumber > 1){
			...
			footerTR.append("<img src=\"" + siteBean.getRootSiteURL() + "/images/icons/firstPage.png\" border=\"0\" title=\"Primera Pagina\" />");
			footerTR.append("</a>");
			
			footerTR.append("<a href=\"#footerPage\" id=\"footerPage\" onclick=\"" + customAjaxPagingFunctionName + "(" + (pageNumber - 1) + ")\">");
			footerTR.append("<img src=\"" + siteBean.getRootSiteURL() + "/images/icons/prevPage.png\" border=\"0\" title=\"Pagina Anterior\" />");
			footerTR.append("</a>");
		}
		...
		if(pageNumber < pagesInvolved){
			footerTR.append("<a href=\"#footerPage\" id=\"footerPage\" onclick=\"" + customAjaxPagingFunctionName + "(" + (pageNumber + 1) + ")\">");
			footerTR.append("<img src=\"" + siteBean.getRootSiteURL() + "/images/icons/nextPage.png\" border=\"0\" title=\"Proxima Pagina\" />");
			footerTR.append("</a>");
			
			footerTR.append("<a href=\"#footerPage\" id=\"footerPage\" onclick=\"" + customAjaxPagingFunctionName + "(" + pagesInvolved + ")\">");
			footerTR.append("<img src=\"" + siteBean.getRootSiteURL() + "/images/icons/lastPage.png\" border=\"0\" title=\"Ultima Pagina\" />");
			footerTR.append("</a>");
		}
