...
public class SendMailUtil {
	...
	private static final String EMAIL_AUTH_USER = "email.auth.user";
	private static final String EMAIL_AUTH_PWD = "email.auth.pwd";
	private static final String EMAIL_ENABLE_DEBUG_LOG = "email.enable.debug.log";
	...
	private static Session getSession(){
		...
		try {
			...
			props.put("mail.smtp.host", AppProperties.getPropertyValue(EMAIL_SMTP_HOST));
			
			//validamos que el puerto tenga formato numerico
			try {
				props.put("mail.smtp.port", Integer.parseInt(AppProperties.getPropertyValue(EMAIL_SMTP_PORT)));
			} catch (Exception e) {
			...
			if(PROTOCOL_SSL.equals(AppProperties.getPropertyValue(EMAIL_PROTOCOL).toUpperCase())){
				props.put("mail.smtp.auth", "true");  // If you need to authenticate
				props.put("mail.smtp.socketFactory.class", "javax.net.ssl.SSLSocketFactory");
				props.put("mail.smtp.socketFactory.fallback", "false");
				
				try {
					props.put("mail.smtp.socketFactory.port", Integer.parseInt(AppProperties.getPropertyValue(EMAIL_SMTP_PORT)));
				} catch (Exception e) {
					// TODO: handle exception
					logger.warn(method + "Se va a usar el puerto por defecto para el envio del correo bajo el protocolo SSL");
				}
			} else if(PROTOCOL_TLS.equals(AppProperties.getPropertyValue(EMAIL_PROTOCOL).toUpperCase())){
				...
				sess = Session.getDefaultInstance(props,
						new Authenticator() {
							protected PasswordAuthentication getPasswordAuthentication() {
								return new PasswordAuthentication(AppProperties.getPropertyValue(EMAIL_AUTH_USER),
										AppProperties.getPropertyValue(EMAIL_AUTH_PWD));
							};
        			});
			} else {
				sess = Session.getDefaultInstance(props);
			...
			if(Boolean.parseBoolean(AppProperties.getPropertyValue(EMAIL_ENABLE_DEBUG_LOG))){
				sess.setDebug(true);
			}
		} catch (Exception e) {
			// TODO: handle exception
	...
	public static boolean sendEmail(List<Address> dirTo){
		...
		if(sess != null){
			try {
				...
				mensaje.setFrom(new InternetAddress(AppProperties.getPropertyValue(EMAIL_AUTH_USER)));
