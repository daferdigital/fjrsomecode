...
function BasicTable($localizador) {
	$sql1 = "SELECT vpc.fecha_compra, lvpc.nro_inscripcion, UPPER(cl.persona_juridica) AS persona_juridica, cl.razon_social, cl.rif, cl.nombre, cl.apellido, cl.ci, cl.direccion, cl.telefono1, cl.telefono2, cl.fax,
	lvpc.nombre_pasajero, lvpc.ci, vpc.fecha_nacimiento, vpc.edad, vpc.nro_pasaporte, vpc.nro_visa, vpc.telefono, vpc.correo, vpc.colegio, vpc.talla_camisa, vpc.ciudad, vpc.direccion,
	lvpc.descripcion, lvpc.cantidad, lvpc.monto_pagar
	...
	$sql1 = "SELECT rec.id, rec.fecha_pago, rec.tipo_pago, rec.moneda, rec.tasa_cambio, rec.monto_pago, rec.equivalente_dolar, 
...
if(isset($_POST["cedula"])){
	...
	$sql1 = "SELECT lvpc.nro_inscripcion FROM linea_ventas_paquetes_credito lvpc WHERE lvpc.ci = '".$_POST["cedula"]."'";
	...
	$pdf->Output();
	mysql_close($dbLink);
} else if(isset($_POST["localizador"])){
	$pdf = new PDF();
	$pdf->AddPage();
	$pdf->BasicTable($_POST["localizador"]);
	$pdf->Output();
} else {
