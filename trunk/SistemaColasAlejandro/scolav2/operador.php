<?Php 
	...
	if($_GET['habilitar']){
		DBUtil::executeQuery("UPDATE ticket SET estado=4 WHERE estado=5 AND id='".$_GET['habilitar']."' LIMIT 1");
?>
		<script type="text/javascript">
			jQuery(document).ready(function(){
				jQuery('#mensaje_eventos').html('Ticket habilitado con �xito');
				jQuery('#mensaje_eventos').show('slow');
				setTimeout("jQuery('#mensaje_eventos').hide('slow');",5000);
			});
		</script>
<?Php
	} // end if($_GET['habilitar']){
	
	if($_GET['anular']){
		DBUtil::executeQuery("update ticket set estado=5, fecha_atencion=NOW() WHERE id='".$_GET['anular']."' LIMIT 1");
	}
...
?>
    ...
    <table align="left" width="300px">
        <tr>
        	<td align="right"><label class="subtit_form">Departamento: </label></td>
        	<td><?Php echo $_SESSION['datos']['descripcion_departamento'];?></td>
        </tr>
        <tr>
        	<td align="right"><label class="subtit_form">Taquilla: </label></td>
        	<td><?Php echo $_SESSION['datos']['descripcion_taquilla'];?></td>
        </tr>
        <tr>
        	<td align="right"><label class="subtit_form">Operador: </label></td>
        	<td><?Php echo $_SESSION['datos']['nombre'];?></td>
        </tr>
        <tr>
        	<td align="right"><label class="subtit_form">Cedula: </label></td>
        	<td><?Php echo $_SESSION['datos']['cedula'];?></td>
        </tr>
	</table>
    <table width="500px">
    	<tr>
    		<td align="right">
    			Si desea habilitar un ticket anulado presione 
    			<label onclick="javascript:jQuery('#hab_ta').show('slow');">
    				<b>AQU&Iacute;</b>
    			</label>
    		</td>
    	</tr>
        <tr style="display:none;" id="hab_ta">
        	<td align="right" bgcolor="#990000" style="color:#FFF;">
        		<strong>Indique el N&deg; de serial: </strong>
        		<input type="text" name="ticket_anulado" value="" />
        		<input type="button" value="Habilitar" onclick="javascript: if(jQuery('[name=\'ticket_anulado\']').val()){location.href='?habilitar='+jQuery('[name=\'ticket_anulado\']').val();}else{alert('Indique el serial del ticket');}" />
        	</td>
        </tr>
	</table>
	...
	<div style="width:800px; height:40px; position:relative; clear:both;">
        <div align="left" style="float:left; width:400px;">
	    	<label class="subtit_form" style="color:#00F;">
	    		Atendiendo al ticket n&uacute;mero: 
	    	</label>
	    	<label id="anumero" class="anumero" style="color:#F00;">0</label>
	...
	<p>
	  <?Php 
		DBUtil::executeQuery("set @atendidos=0,@total_valido=0,@estatus_tic='',@ultimo_atendido='00:00:00'");
		$sql_taquillas="
						select 
								*,
		...
		$query_taquillas=DBUtil::executeSelect($sql_taquillas) or die(mysql_error());
		$num_taquillas=count($query_taquillas);
		if($num_taquillas>0){				
?>
	</p>
	<p>&nbsp;</p>
	...
	<div id="terminal" >
	<?Php 
		$pares=0; $contador=1; $uatendido=0;
	?>
		<table width="98%" border="1" class="tam_tabla" align="center" cellspacing="0" cellpadding="5" style=" border-collapse:collapse;">
        	<tr class="tit_filas">
        		...
        		<td width="17%">Hora</td>
        		<td width="27%">Operador</td>
        		<td width="25%">Estatus</td>
        		<td width="21%">Acciones</td>
        	</tr>
			<?Php
				foreach($query_taquillas as $var_taquillas){	
			?>
				<tr class="tr">
					<td><?Php echo $var_taquillas['correlativo']; ///$contador; ?></td>
					<td align="center"><?Php echo $var_taquillas['hora'].' / '.$var_taquillas['hora_atendido']; ?></td>
					<td><?Php echo $var_taquillas['nombre']; ?></td>
					<td align="center">
						<?Php echo $var_taquillas['estatus_ticket']; ?> | 
