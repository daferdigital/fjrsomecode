...
public final class SendCommandUtil {
	...
	/**
	 * Metodo para ejecutar varios comandos AT en el puerto indicado.
	 * 
	 * @param port Puerto en el cual se ejecutara el comando indicado
	...
	public static String sendCommandsToPort(CommPortIdentifier port, List<ATCommandDTO> commands){
		...
		try{
			...
			for (ATCommandDTO atCommand : commands) {
				...
				response = sb.toString();
				DebugLog.info("Respuesta del comando [" + atCommand + "] en el puerto [" + port.getName() 
						+ "] fue: " + response.replaceAll("\n", "").replaceAll("\r", ""));
			}
		} catch (PortInUseException e) {
			DebugLog.error("El puerto [" + port.getName() + "]. se encuentra en uso (seguramente es el asociado a la conexion de datos): " 
			...
			// TODO: handle exception
			DebugLog.error("Error durante la ejecucion del comando en el puerto [" + port.getName() 
