<?php session_start();
	...
	//$_POST=convertArrayKeysToUtf82($_POST);
		...
		//inserto datos de apuestas equipo A
			...
			for($i=0;$i<count($arraids);$i++){
				...
				switch($arraids[$i]){
					...
					break;
					case '96': $mult=($_POST['vm'.$arraids[$i]]!=''?$sig.$_POST['vm'.$arraids[$i]]:'1');
					break;
					case '97': $mult=($_POST['vm'.$arraids[$i]]!=''?$sig.$_POST['vm'.$arraids[$i]]:'1');
					break;
					case '108': $mult=($_POST['vm'.$arraids[$i]]!=''?$sig.$_POST['vm'.$arraids[$i]]:'1');
					break;
				}
		...
		//inserto datos de apuestas equipo B
			...
			mysql_query("update taquillas set actualizar='1' where actualizar='0'");
?>