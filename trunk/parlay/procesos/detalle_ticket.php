...
<?Php
	...
	//genero archivo de impresion
			echo $select_imp="select *,date_format(hora,'%r') as hora_j,date_format(fecha_venta,'%d/%m/%Y') as fecha_j,date_format(hora_juego,'%r') as hora_l from vista_ventas_detalles where idventa='".$_GET['idticket']."' $concat order by hora_juego";
			$query_imp=mysql_query($select_imp);
				if(mysql_num_rows($query_imp)>0){
					...
					while($var_imp=mysql_fetch_assoc($query_imp)){
						...
						if($cont==0){
							?>
                            	<tr><td colspan="4" align="left" bgcolor="#CCCCCC"  class="encab"><strong>FECHA: <?Php echo $var_imp['fecha_j']."   ";?> HORA <?Php echo $var_imp['hora_j'];?></strong></td></tr>
                            	  <tr>
                                    <td align="left" class="encab"><strong>NÚMERO DE TICKET: </strong></td>
                                  ...
                                  <tr>
                                    <td width="157" align="left" class="encab"><strong>MONTO APOSTADO:</strong></td>
                                    <td colspan="2" class="encab"><strong>
                                    <?Php $ap2=$var_imp['apuesta']; echo $apuesta=number_format($var_imp['apuesta'],2);?>
                                    </strong></td>
                                    <td width="75" align="right" class="td_bottom">&nbsp;</td>
                                  </tr>
                                  <tr>
                                    <td align="left" class="td_bottom">&nbsp;</td>
                                    <td width="102" class="td_bottom">&nbsp;</td>
                                    <td width="67" class="td_bottom">&nbsp;</td>
						...
						?>
                        	<tr>
                        	  <td colspan="2" height="25"><?Php echo $var_imp['hora_l'];?> <?Php echo substr($var_imp['nombre_categoria'],0,4);?>&nbsp;&nbsp;&nbsp;<?Php echo $var_imp['nombre_equipo'] .' ('.$var_imp['descripcion_apuesta'].')';?></td>
                        	  <td><div align="center">
                        	    <?Php 
					...
					?>
                          ...
                          <tr>
                    	<td colspan="2" class="td_top">INVERSIÓN BsF</td><td class="td_top">:</td>
