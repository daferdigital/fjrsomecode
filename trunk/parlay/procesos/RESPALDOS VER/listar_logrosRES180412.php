				...
				while($varlistado=mysql_fetch_assoc($queryclistado)){
					...
					?>
						<tr class="tr pag<?Php echo $npagl;?>" style="<?Php echo $ocultarfilas;?>">
                            ...
                            <td align="right"><img src="imagenes/editar.png" width="16px" border="0" onClick="javascript:$('html, body').animate({ scrollTop: 0 }, 0);deshacer(cadena_hiden);$('[name=\'guardar\']').val('Modificar');editar_datos('idlogro,fecha,hora,visitante,home,idlogro_equipoA,idlogro_equipoB,referenciaA,referenciaB,estatus','<?php echo $varlistado['idlogro'].','.$varlistado['fecha_logro'].','.$varlistado['hora'].','.$equipoA.','.$equipoB.','.$idlogroeA.','.$idlogroeB.','.$referenciaA.','.$referenciaB.','.$varlistado['estatus']; ?>');editar_datos_apuestas('<?Php echo $cadenaids2;?>','<?php echo $valoresids;?>','idapuesta');$('#visitante').change();$('#home').change();editar_datos('lvisitante,lhome','<?php echo $lanzadorA.','.$lanzadorB;?>');editar_datos_radio('<?Php echo $cadenaids2;?>','<?php echo $valoresmult;?>','m');" /><!--<img style="padding-left:5px;" src="imagenes/<?php echo ($varlistado['estatus']=='1'?'eliminar.png':'BIEN.png'); ?>" width="16px" border="0" onClick="if(confirm('Presione aceptar si desea <?php echo ($varlistado['estatus']=='1'?'Deshabilitar':'Habilitar'); ?> el registro <?php echo $varlistado['idlogro']; ?> ')){location.href='procesos/habilita_deshabilita.php?equipo=logros&deshab=<?php echo ($varlistado['estatus']);?>&id=<?php echo $varlistado['idlogro']; ?>&redir=logros_beisbol.php';}" />--></td></tr>
					<?Php
					$c++;
				}
				?></table><br />
