			...
			if(mysql_num_rows($query)>0){
				...
				switch($tabla){
					...
					case 'vista_taquillas':
						...
						/*Verifico el enlace de la taquilla a la pc*/
						
						/*
						if($var['generar_cookie']==1){
							$fecha=mktime(0,0,0,1,1,2013);
							$valor="vAlTaQui".rand(10,999);
							//setcookie("taquilla", $valor, $fecha, "/", "www.ingenieriadesistemas.com.ve", 0); 
							setcookie("taquilla", $valor, $fecha, "/", "", 0); 
							mysql_query("update taquillas set valor_cookie='".$valor."',generar_cookie='0' where idtaquilla='".$var['idtaquilla']."' limit 1");
						}elseif($_COOKIE['taquilla']!=$var['valor_cookie']){
							$_SESSION['menu']='no';
						}
						*/
					break;
