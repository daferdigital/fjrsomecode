...
<?Php
	include("conexion.php");	
		if($_POST['apuesta'] && $_SESSION['vender']=='si'){	
			...
			$cadena=sprintf("insert into ventas() values('','".$_SESSION['datos']['idtaquilla']."','".$_POST['fecha']."',DATE_ADD('".$_POST['fecha']."', INTERVAL 3 DAY),'".date('H:i:s')."','%s','%s','0','0','0','0','0','".rand(10000000,99999999)."','".$codigo_ticket."','0','%s','1')",mysql_escape_string($_POST['monto_apuesta']),mysql_escape_string($_POST['monto_pagar']),mysql_escape_string($_POST['num_apuestas']));
			...
			$query_imp=mysql_query($select_imp);
				if(mysql_num_rows($query_imp)>0){
					...
					?>
                    <tr>
                    	<td colspan="2" class="td_top">INVERSIÓN<? echo "  ";?>BsF:</td>
                    	<td class="td_top">&nbsp;</td><td align="right" class="td_top"><?Php echo $apuesta;?></td>
                    </tr>
                    <tr>
