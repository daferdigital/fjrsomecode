<?Php
	...
	function mysql_debug_query($query){
		...
		if(mysql_error()) 
			echo "ERROR!!!! --->>> Query: $query - Error: ".mysql_error();
		return $ret;
	}
	...
	return $array;
	  ...
	  function genera_aciertos($idcategoria_apuesta,$idlogro_equipo){
		  ...
		  if($_SESSION['suspendido']>0):
		  	...
		  	mysql_debug_query("update logros_equipos_categorias_apuestas set suspendido=1 where idcategoria_apuesta='".$idcategoria_apuesta."' and idlogro_equipo='".$idlogro_equipo."'");

		  else:

		  	mysql_debug_query("update logros_equipos_categorias_apuestas set suspendido=0 where idcategoria_apuesta='".$idcategoria_apuesta."' and idlogro_equipo='".$idlogro_equipo."'");
			...
			$idsmediojuego=explode(",",$idsmediojuego);
				...
				for($f=0;$f<count($idsmediojuego);$f++){
					...
					if(!$existe)
						...
						mysql_debug_query("insert into logros_equipos_categorias_apuestas_banqueros_aciertos() values('','".$idsmediojuego[$f]."','1')");

					else

						mysql_debug_query("update logros_equipos_categorias_apuestas_banqueros_aciertos set estatus='1' where idlogro_equipo_categoria_apuesta_banquero='".$idsmediojuego[$f]."' limit 1");	
		
					if(mysql_error())
						echo "\n<br>logros error: ".mysql_error();
				}
	...
	function genera_encabezado($que_muestro,$categoria,$deporte=''){
		...
		global $array_datos,$equipoA,$equipoB,$hora,$color,$cuenta_juego,$varlogros,$imagenA,$imagenB,$total_juegos,$refA,$refB,$pitcherA,$pitcherB,$ancho,$soloimp;
		...
		switch($categoria){
			...
			break;
						...
						////futbol americano aqui

			

						case '6': //Futbol AMERICANO
				...
				?>
					...
					<table width="<? echo $ancho;?>" class="ventas_taquilla" border="1" cellspacing="0" cellpadding="3">
						...
						<tr class="titulo_tablas"><td colspan="10" align="center">Logros de <b><?Php echo $deporte;?></b> al <?Php list($ano,$mes,$dia)=explode("-",$_REQUEST['fecha']); echo "$dia/$mes/$ano";?></td></tr>
						...
						<td colspan="2">Alta o Baja</td><td>Empate</td></tr>
				...
				elseif($que_muestro=='impresion'): 
					...
					?><tr class="borde_rigth_bottom_top"  bgcolor="<?Php echo $color;?>">
                        ...
                        <td align="right"><?Php if($array_datos[100][1]):?><?php echo $array_datos[100][1] ?><?php endif;?></td>

                        <td align="left"><?Php if($array_datos[101][1]):?><?php echo $array_datos[101][1] ?><?php endif;?></td>
                        ...
                        <tr class="borde_rigth_bottom" bgcolor="<?Php echo $color;?>">
                            ...
                            <td align="right"><?Php if($array_datos[111][1]):?><?php echo $array_datos[111][1] ?><?php endif;?></td>

                            <td align="left"><?Php if($array_datos[99][1]):?><?php echo $array_datos[99][1] ?><?php endif;?></td>
				...
				elseif($que_muestro=='items_ventas'):	
						...
						switch($varlogros['idcategoria_apuesta']){							
							...
							case '102': $tp='AJC';$equipo_apuesta=obtener_mini_contrincantes($varlogros['idlogro_equipo'])/*$varlogros['nombre_equipo']*/; $mult='>'.str_replace(".0","",$varlogros['multiplicando']);

							break;

							case '103': $tp='BJC';$equipo_apuesta=obtener_mini_contrincantes($varlogros['idlogro_equipo'])/*$varlogros['nombre_equipo']*/; $mult='<'.str_replace(".0","",$varlogros['multiplicando']);

							break;

							case '104': $tp='AMJ';$equipo_apuesta=obtener_mini_contrincantes($varlogros['idlogro_equipo'])/*$varlogros['nombre_equipo']*/; $mult='>'.str_replace(".0","",$varlogros['multiplicando']);

							break;

							case '105': $tp='BMJ';$equipo_apuesta=obtener_mini_contrincantes($varlogros['idlogro_equipo'])/*$varlogros['nombre_equipo']*/; $mult='<'.str_replace(".0","",$varlogros['multiplicando']);
