...
<?Php
	include("conexion.php");	
		if($_POST['apuesta'] && $_SESSION['vender']=='si'){	
			...
			$cadena=sprintf("insert into ventas() values('','".$_SESSION['datos']['idtaquilla']."','".$_POST['fecha']."',DATE_ADD('".$_POST['fecha']."', INTERVAL 3 DAY),'".date('H:i:s')."','%s','%s','0','0','0','0','0','0','0','0','0','".rand(10000000,99999999)."','".$codigo_ticket."','0','%s','','','','1')",mysql_escape_string($_POST['monto_apuesta']),mysql_escape_string($_POST['monto_pagar']),mysql_escape_string($_POST['num_apuestas']));
				...
				if(mysql_num_rows($query_imp)>0){
					...
					while($var_imp=mysql_fetch_assoc($query_imp)){
						if($cont==0){
							?>
                            	  <tr>
                                    <td colspan="4" align="left" style="font-size:14px;">TICKET: <?Php echo str_pad($var_imp['codigo_ticket'],8,'0',STR_PAD_LEFT);?></td>
                                  </tr>
                                  <tr>
