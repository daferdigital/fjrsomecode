...
include("conexion.php");
	if($_REQUEST['codigo_ticket']){
		if($_SESSION['perfil']==1){//Administrador
			$select="select * from vista_ventas where fecha_prorroga>='".date('Y-m-d')."' and (ganador='1' or reembolsar='1') and vencido='0' and reembolsado=0 and codigo_ticket='".(int)($_REQUEST['codigo_ticket']*1)."' and codigo_cliente='".(int)($_REQUEST['codigo_cliente']*1)."' limit 1";
		}elseif($_SESSION['perfil']==2){//banquero
			$select="select * from vista_ventas where ".$_SESSION['nombre_idtabla']."='".$_SESSION['datos'][$_SESSION['nombre_idtabla']]."' and (ganador='1' or reembolsar='1') and reembolsado=0 and vencido='0' and fecha_prorroga>='".date('Y-m-d')."' and  codigo_ticket='".(int)($_REQUEST['codigo_ticket']*1)."' and codigo_cliente='".(int)($_REQUEST['codigo_cliente']*1)."' limit 1";
