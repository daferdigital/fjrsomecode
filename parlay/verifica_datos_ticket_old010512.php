			...
			$query=mysql_query($select);
				if(mysql_num_rows($query)>0){
					...
					if($tick['pagado']==1){
						...
						$jsondata['estatus_ticket'] =  "El ticket es GANADOR y no ha sido pagado, debe pagar un total de ".$tick['total_ganar'].' BsF.<br><center><input type="button" class="boton" value="Pagar" onclick="javascript: mostrarmens=\'si\';comentario=\'Indique tanto código del ticket como código del cliente\'; nolistado=\'no\'; validar(document.pagar,\'pagar_ticket.php\');">  <input type="reset" class="boton" value="Limpiar campo" onclick="javascript:$(\'#bot_cons\').show(\'slow\');$(\'#detalle_del_ticket\').html(\'\');">';
