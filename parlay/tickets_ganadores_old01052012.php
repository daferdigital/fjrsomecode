	...
	$query_ventas=mysql_query($sql_ventas);
		if(mysql_num_rows($query_ventas)>0){ $contador=1;
			...
			?>
                ...
                </td><td><?Php echo $var_sql['fecha_venta'].' - '.$var_sql['hora'];?></td><td><?Php echo $var_sql['fecha_prorroga'];?></td><td align="right"><?Php echo $var_sql['apuesta'];?> BsF.</td><td align="right"><?Php echo $var_sql['total_ganar'];?> BsF.</td><td align="right"><?Php echo $var_sql['monto_real_pagar'];?> BsF.</td><td>
                	<?Php
						if($var_sql['anulado']):
