...
<form name="categorias" method="post" action="procesos/guardar_categorias.php">
	...
	<table width="100%" cellpadding="4px" cellspacing="0">
    	<tr valign="top">
        	<td width="50%"><label class="tit_campos">Nombre del Deporte:</label> <input type="text" name="nombre" id="nombre" value="" /> <label class="campo_obligatorio">*</label></td>
					...
					$numap=mysql_num_rows($queryap);
						...
						?><input type="hidden" name="nregistros" value="<?php echo $numap; ?>" /><?Php
							while($varap=mysql_fetch_assoc($queryap)){
								...
								?>
									<tr class="tr"><td><input type="checkbox" name="idapuesta<?php echo $contador; ?>" id="idapuesta<?php echo $varap['idapuesta']; ?>" value="<?php echo $varap['idapuesta']; ?>" /></td><td><?Php echo $varap['nombre'].' / '.$varap['descripcion'];?></td></tr>                                    
								<?Php
								$contador++;
							}
						}else{
