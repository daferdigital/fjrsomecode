...
class GanadoresBasket{
	...
	/**
	 * Metodo para verificar si determinada venta de una apuesta del tipo A Ganar Juego Completo en basket
	 * resulto ganadora o perdedora.
	 *
	 * @param $idventa, codigo del ticket (id de la venta en base de datos)
	 */
	private static function checkAGanarBasketJuegoCompleto($rowVistaVentasDetalles){
	...
	/**
	 * Metodo para verificar si determinada venta de una apuesta del tipo A Ganar Medio Juego en basket
	 * resulto ganadora o perdedora.
	 *
	 * @param $idventa, codigo del ticket (id de la venta en base de datos)
	 */
	private static function checkAGanarBasketMedioJuego($rowVistaVentasDetalles){
		$codeReturn = VentasDAO::$RESULTADO_PERDEDOR;
		...
		if(count($result) > 0){
			...
			//tengo el resultado del equipo, veo en base a la categoria de la apuesta si se gano o no
			if($rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_AGANAR_MJ_A){
				//comparando apuesta de ganador el equipo A
				$compara= $resultadoEquipoA;
				BitacoraDAO::registrarComentario("[".$rowVistaVentasDetalles["idventa_detalle"]."] Apuesta equipoA"
						.$resultadoEquipoA." + ".$rowVistaVentasDetalles['multiplicando']." = ".$compara);
	...
	/**
	 * Metodo para verificar si determinada venta de una apuesta del tipo A Ganar 2da Mitad en basket
	 * resulto ganadora o perdedora.
	 *
	 * @param $idventa, codigo del ticket (id de la venta en base de datos)
	 */
	private static function checkAGanar2daMitad($rowVistaVentasDetalles){
		$codeReturn = VentasDAO::$RESULTADO_PERDEDOR;
		...
		if(count($result) > 0){
			...
			//tengo el resultado del equipo, veo en base a la categoria de la apuesta si se gano o no
			if($rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_AGANAR_2DA_MITAD_A){
				//comparando apuesta de ganador el equipo A
				$compara= $resultadoEquipoA;
				BitacoraDAO::registrarComentario("[".$rowVistaVentasDetalles["idventa_detalle"]."] Apuesta equipoA"
						.$resultadoEquipoA." + ".$rowVistaVentasDetalles['multiplicando']." = ".$compara);
	...
	/**
	 * Metodo para verificar si determinada venta de una apuesta del tipo Primer Tiempo en basket
	 * resulto ganadora o perdedora.
	 *
	 * @param $idventa, codigo del ticket (id de la venta en base de datos)
	 */
	private static function checkPrimerTiempoBasket($rowVistaVentasDetalles){
		$codeReturn = VentasDAO::$RESULTADO_PERDEDOR;
		...
		."WHERE le.idlogro = ".$rowVistaVentasDetalles["idlogro"]
		." AND le.idlogro_equipo = lecr.idlogro_equipo "
		."AND (lecr.idcategoria_resultado = 23 OR lecr.idcategoria_resultado = 24)";
		...
		if(count($result) > 0){
			...
			$resultadoEquipoB = 0;
			foreach ($result as $row){
				if($row["idcategoria_resultado"] == 23){
					$resultadoEquipoA = $row["resultado"];
				}
				if($row["idcategoria_resultado"] == 24){
					$resultadoEquipoB = $row["resultado"];
				}
			}
			...
			//tengo el resultado del equipo, veo en base a la categoria de la apuesta si se gano o no
			if($rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_PRIMER_TIEMPO_A){
				//comparando apuesta de ganador el equipo A
				$compara= $resultadoEquipoA;
				BitacoraDAO::registrarComentario("[".$rowVistaVentasDetalles["idventa_detalle"]."] Apuesta equipoA"
						.$resultadoEquipoA." + ".$rowVistaVentasDetalles['multiplicando']." = ".$compara);
	...
	/**
	 * Metodo para verificar si determinada venta de una apuesta del tipo Segundo Tiempo en basket
	 * resulto ganadora o perdedora.
	 *
	 * @param $idventa, codigo del ticket (id de la venta en base de datos)
	 */
	private static function checkSegundoTiempoBasket($rowVistaVentasDetalles){
		$codeReturn = VentasDAO::$RESULTADO_PERDEDOR;
		...
		."WHERE le.idlogro = ".$rowVistaVentasDetalles["idlogro"]
		." AND le.idlogro_equipo = lecr.idlogro_equipo "
		."AND (lecr.idcategoria_resultado = 25 OR lecr.idcategoria_resultado = 26)";
		...
		if(count($result) > 0){
			...
			$resultadoEquipoB = 0;
			foreach ($result as $row){
				if($row["idcategoria_resultado"] == 25){
					$resultadoEquipoA = $row["resultado"];
				}
				if($row["idcategoria_resultado"] == 26){
					$resultadoEquipoB = $row["resultado"];
				}
			}
			...
			//tengo el resultado del equipo, veo en base a la categoria de la apuesta si se gano o no
			if($rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_SEGUNDO_TIEMPO_A){
				//comparando apuesta de ganador el equipo A
				$compara= $resultadoEquipoA;
				BitacoraDAO::registrarComentario("[".$rowVistaVentasDetalles["idventa_detalle"]."] Apuesta equipoA"
						.$resultadoEquipoA." + ".$rowVistaVentasDetalles['multiplicando']." = ".$compara);
	...
	/**
	 * Metodo para verificar si determinada venta de una apuesta del tipo Tercer Tiempo en basket
	 * resulto ganadora o perdedora.
	 *
	 * @param $idventa, codigo del ticket (id de la venta en base de datos)
	 */
	private static function checkTercerTiempoBasket($rowVistaVentasDetalles){
		$codeReturn = VentasDAO::$RESULTADO_PERDEDOR;
		...
		."WHERE le.idlogro = ".$rowVistaVentasDetalles["idlogro"]
		." AND le.idlogro_equipo = lecr.idlogro_equipo "
		."AND (lecr.idcategoria_resultado = 27 OR lecr.idcategoria_resultado = 28)";
		...
		if(count($result) > 0){
			...
			$resultadoEquipoB = 0;
			foreach ($result as $row){
				if($row["idcategoria_resultado"] == 27){
					$resultadoEquipoA = $row["resultado"];
				}
				if($row["idcategoria_resultado"] == 28){
					$resultadoEquipoB = $row["resultado"];
				}
			}
			...
			//tengo el resultado del equipo, veo en base a la categoria de la apuesta si se gano o no
			if($rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_TERCER_TIEMPO_A){
				//comparando apuesta de ganador el equipo A
				$compara= $resultadoEquipoA;
				BitacoraDAO::registrarComentario("[".$rowVistaVentasDetalles["idventa_detalle"]."] Apuesta equipoA"
						.$resultadoEquipoA." + ".$rowVistaVentasDetalles['multiplicando']." = ".$compara);
	...
	private static function checkAltaBajaSextoBasket($rowVistaVentasDetalles){
		...
		if(count($result) > 0){
			...
			BitacoraDAO::registrarComentario("[".$rowVistaVentasDetalles["idventa_detalle"]."] "
					."Resultado altas-bajas 2H (equipoA/equipoB) (".$resultadoEquipoA."/".$resultadoEquipoB.") "
					."Mi multiplicando -> ".$rowVistaVentasDetalles['multiplicando']);
	...
	public static function calcularGanador($rowVistaVentasDetalles){
		...
		} else if($rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_AGANAR_2DA_MITAD_A
				|| $rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_AGANAR_2DA_MITAD_B){
		...
		} else if($rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_PRIMER_TIEMPO_A
				|| $rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_PRIMER_TIEMPO_B){
			// es A GANAR PRIMER TIEMPO de basket
			$codeReturn = GanadoresBasket::checkPrimerTiempoBasket($rowVistaVentasDetalles);
		} else if($rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_SEGUNDO_TIEMPO_A
				|| $rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_SEGUNDO_TIEMPO_B){
			// es A GANAR SEGUNDO TIEMPO de basket
			$codeReturn = GanadoresBasket::checkSegundoTiempoBasket($rowVistaVentasDetalles);
		} else if($rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_TERCER_TIEMPO_A
				|| $rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_TERCER_TIEMPO_B){
			// es A GANAR TERCER TIEMPO de basket
			$codeReturn = GanadoresBasket::checkTercerTiempoBasket($rowVistaVentasDetalles);
		} else if($rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_ALTA_SEXTO_A
				|| $rowVistaVentasDetalles["idcategoria_apuesta"] == GanadoresBasket::$BASKET_BAJA_SEXTO_A){
