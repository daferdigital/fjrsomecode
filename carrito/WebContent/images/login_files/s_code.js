...
s.formList="" //Definir aquí los nombres de los formularios a analizar
...
function s_doPlugins(s) {
	...
	if(s.pageName==undefined || s.pageName==null || s.pageName==""){
		/* Pagename: deben especificarse en el array los parámetros que se quieren conservar */
		var params = []; // ningún parámetro
	...
	s.prop16=s.eVar16=s.getTimeParting('h','+1',new Date().getFullYear()); // Set hour 
	s.prop17=s.eVar17=s.getTimeParting('d','+1',new Date().getFullYear()); // Set day 
	s.prop18=s.eVar18=s.getTimeParting('w','+1',new Date().getFullYear()); // Set weekday 
		
	s.eVar26=s.getQueryParam('intcmp');
	...
	if (s.getQueryParam('s_kw','cid')) {
		s.eVar49=s.getQueryParam('s_kw')
		s.eVar49=s.rep(s.eVar49,'%20',' ')};
	
	/* Llamada a FormAnalysis */
	s.setupFormAnalysis();
}
...
/* Envio de evento onClick Modulos de apoyo */ 
function envioSCModulosApoyo(nombreBoton){ 
	s=s_gi(s_account); 
	s.linkTrackVars="prop57,eVar57,events"; 
	s.linkTrackEvents="event53"; 
	s.prop57=s.pageName + ":" +nombreBoton; 
	s.eVar57=s.prop57; 
	s.events="event53"; 
	s.tl(this,'o',s.prop57); 
} 
/* Función para medición de tipo, posicion y titulo de los módulos */ 
...
/*
 * Plugin: cleanUrlParameters 1.0 - Removes specific params from url
 */
s.url_params_select=new Function("url","params_to_keep",""
+"var parts=s.url_split(url);parts[0]=s.url_index(parts[0]);var param"
+"s_text=parts[1];if(!params_text){return s.url_join(parts[0],parts[1"
+"],parts[2]);}var params_hash=s.params_to_hash(s.split(params_text,'"
+"&'));var new_params='';for(var i in params_to_keep){par=params_to_k"
+"eep[i];if(params_hash[par]){if(new_params=='')new_params+=(par+'='+"
+"params_hash[par]);else new_params+=('&'+par+'='+params_hash[par]);}"
+"}return s.url_join(parts[0],new_params,parts[2]);");
s.url_split=new Function("url",""
+"var anchor=s.split(url,'#')[1];var url_parts=s.split(s.split(url,'#"
+"')[0],'?');var url_start=url_parts[0];var params=url_parts[1];retur"
+"n [s.main_domain(url_start),params,anchor];"); 
s.main_domain=new Function("url",""
+"var m=url.replace('http://','').replace('https://',''); var splitte"
+"d=s.split(m,'/');var domain=splitted[0]; var splitted_domain=s.spli"
+"t(domain,'.');if(splitted_domain.length > 1){ domain=splitted_domai"
+"n[splitted_domain.length-2];}if(splitted.slice(1).length===0){retur"
+"n domain;}return domain+'/'+splitted.slice(1).join('/');");
s.url_join=new Function("url","params","anchor",""
+"if(params){return url+'?'+params}else if(params){return url+'?'+par"
+"ams;}else{return url;}");
s.params_to_hash=new Function("params",""
+"var params_hash={};var par = null;var par_pair=null;for(var i in pa"
+"rams){if(params[i].substring){par_pair=s.split(params[i],'=');param"
+"s_hash[par_pair[0]]=par_pair[1];}}return params_hash;");
s.url_index=new Function("url",""
+"var n=(url.indexOf('.htm')<0&&url.indexOf('.php')<0)?(url+=(url[url"
+".length-1]=='/')?('index.html'):('/index.html')):url;return n;");
/*
...
+ "rue':t.substring(i+1);if(p.toLowerCase()==k.toLowerCase()){v=s.rep(v"
+ ",'+',' '); return v;}}return ''");
/*
