...
class DBUtil {
	private static $storeErrors = true;
	private static $storeResults = false;
	...
	private static function storeError($queryOperation, $result){
		if(DBUtil::$storeErrors == true){
			...
			." VALUES(now(),'".mysql_real_escape_string($queryOperation, $dbConObj->getConnection())."','".mysql_real_escape_string($result, $dbConObj->getConnection())."','1',".(time() - $time0).",".$idUsuario.")";
